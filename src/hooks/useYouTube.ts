...
export const useChannelVideos = (channelId: string, enabled: boolean = true) => {
  const { tokens, isAuthenticated } = useAuth();
...
export const useVideoSearch = (query: string, enabled: boolean = true) => {
  ...
  return useInfiniteQuery({
    ...
    initialPageParam: undefined as string | undefined,
    getNextPageParam: (lastPage) => lastPage.nextPageToken,
    enabled: enabled && isAuthenticated && !!tokens?.accessToken && query.length > 0,
...
export const useChannelDetails = (channelId: string, enabled: boolean = true) => {
  const { tokens, isAuthenticated } = useAuth();
